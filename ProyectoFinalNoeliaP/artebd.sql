...
CREATE TABLE obra (
  ...
  constraint fk_obra_autor FOREIGN KEY (autor_id) REFERENCES usuario(idusu) on delete no action on update cascade
);
...
CREATE TABLE puntuacion (
  ...
  constraint fk_puntuacion_obra FOREIGN KEY (obra_id) REFERENCES obra(idobra) on delete no action on update cascade
);
...
insert into puntuacion
    (obra_id, puntuacion, comentario)
values
    (1, 2, 'No se sabe donde está la sonrisa, fatal'),
    (4, 10, 'Empatizo mucho con el autor, ha lagrado transmitir sus sentimientos de una manera magistral');
